      ...
      return (
        <>
          <ListItem
            ...
            className={`${classes.listItem} ${level === 0 ? classes.rootLevel : classes.subLevel}`}
            selected={active ? true : false}
            level={level}
          >
            ...
            {listItemText}
            {collapsedIndex[level] === index ? <Icon>expand_less</Icon> : <Icon>expand_more</Icon>}
          </ListItem>
          <Collapse in={opened ? true : collapsedIndex[level] === index} timeout="auto" unmountOnExit>
            <List className={`${classes.subList} ${icon ? 'withIcon' : null}`} component="div" disablePadding>
